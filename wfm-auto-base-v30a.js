...
A.OVERLAY_ID='wfm-auto-overlay-v33';
...
A.running=false;A.done=false;A.CLICK_THROUGH_SCAN=true;A.transitionMediaTime=0;A.transitionPaintTimer=null;
...
A.preloadVideo=()=>{try{let v=document.getElementById('wfm-auto-preload-video-v33');if(v)return v;v=document.createElement('video');v.id='wfm-auto-preload-video-v33';v.src=A.VIDEO_URL;v.preload='auto';v.playsInline=true;v.muted=true;v.defaultMuted=true;v.volume=0;v.style.cssText='position:fixed;width:1px;height:1px;left:-20px;top:-20px;opacity:.001;pointer-events:none';(document.body||document.documentElement).appendChild(v);try{v.load()}catch(_){}return v}catch(_){return null}};
...
A.overlayTemplate=d=>{
  ...
  const video=root.querySelector('[data-wfm-video]');if(video){video.src=A.VIDEO_URL;video.preload='auto';video.muted=true;video.defaultMuted=true;video.volume=0;video.loop=false;video.playsInline=true;try{video.load()}catch(_){}}
...
A.paintOverlay=()=>{try{
  ...
  const blockingBase='position:fixed;background:#1f2937;pointer-events:auto;margin:0;padding:0;border:0;z-index:1;';
  ...
  frame.style.cssText=`position:fixed;left:${r.left}px;top:${r.top}px;width:${r.width}px;height:${r.height}px;background:transparent;border:1px solid rgba(219,234,254,.62);border-radius:${radius}px;box-shadow:0 0 0 1px rgba(255,255,255,.07),0 0 30px rgba(147,197,253,.18),0 18px 52px rgba(0,0,0,.34),0 0 0 9999px #1f2937;pointer-events:none;z-index:2;`;
}catch(_){} };
...
A.setMode=mode=>{A.ui.mode=mode||'login';if(A.ui.mode==='processing'){if(!A.transitionPaintTimer)A.transitionPaintTimer=setInterval(()=>A.paintOverlay(),16)}else if(A.transitionPaintTimer){clearInterval(A.transitionPaintTimer);A.transitionPaintTimer=null}A.paint();A.report()};
