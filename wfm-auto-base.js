...
A.ui={mode:'login',text:'🌟 Systeem actief! Vul nu je gegevens in en log in. 🌟',detail:'De scannercontroller is actief en blijft tijdens het inloggen op de achtergrond draaien.',kind:'active',progress:null};
...
A.paint=()=>{try{document.body.dataset.mode=A.ui.mode;document.body.dataset.state=A.ui.kind;const s=document.getElementById('state'),d=document.getElementById('detail');if(s)s.textContent=A.ui.text;if(d)d.textContent=A.ui.detail||''}catch(_){}};
...
A.setProgress=stage=>{A.ui.progress=stage||null;A.report()};
A.announce=()=>{try{A.receiver()?.postMessage({type:'wfm-scanner-helper-ready'},A.TEST_ORIGIN)}catch(_){}};
A.announce();A.report();A.announceTimer=setInterval(()=>{A.announce();A.report()},1000);
